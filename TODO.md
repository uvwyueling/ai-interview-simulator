...
### 真机验收（2026-08-27 第一次，部分完成）

- [x] ✅ **等待时长可接受** —— 用户真机实测「编码 + 上传 + 讯飞」的总等待**没有问题**。这是决定要不要把默认值翻到「高准确转写」的唯一依据，在此之前从没在真实条件下判断过
- [x] ✅ **标点是实打实的增益** —— 讯飞会自动加标点，而 Web Speech 的 zh-CN **完全不加**。这一条直接减少用户提交前的手动修改量
- [ ] ⚠️ **转写质量这次没测出来，因为测试条件无效** —— 用户用**示例简历**（虚构前端应届生）却**讲这个产品**（DeepSeek / Supabase / 埋点）。词表从简历+JD 抽取，实际送出的是 `React、ByteSpark、Hackday、TypeScript、Node.js、Python` + 8 个中文词，**三个说到的词一个都不在里面**
  - `deep seek` 没纠正 → 词典无 `deepseek` key，**正确地什么都没做**
  - `买点` 没修好 → dhw 无「埋点」，讯飞没有可用偏置（探针里能修好，正是因为那次把「埋点」放进了热词）
  - **纠正词典零假阳性**，这一点是好消息
  - **重测条件**：用真实简历+JD 且讲简历上写的经历；或把简历换成本产品的技术栈再讲同样的话

- [ ] 🟡 **暴露的真实局限：词表只覆盖简历/JD 里出现过的词** —— 候选人随口提到一个简历上没写的名词时，纠正词典与热词都会**静默失效**。真实面试里通常成立（问的就是你的经历），但简历很短，这种情况不罕见
  - ⚠️ **不建议用「累积用户前几题的回答进词表」来补** —— 那会把 ASR 自己的错误学进去（`买点` 会成为热词），自我强化

- [ ] **仍未做**：多段回答、切题中断、`upgradeStatus` 各分支的真机覆盖；`encodeMs` 的真实分布尚未从埋点回看
